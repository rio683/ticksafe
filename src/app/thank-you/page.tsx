'use client';

import React from 'react';
import Link from 'next/link';
import './thank-you.css';

export default function ThankYouPage() {
    return (
        <main className="thank-you-page">
            <header className="thank-you-header">
                <div className="container" style={{ display: 'flex', justifyContent: 'center' }}>
                    <Link href="https://ticksafenews.com">
                        <img src="/logo-news.png" alt="TickSafe News" style={{ height: '50px' }} />
                    </Link>
                </div>
            </header>

            <section className="thank-you-container">
                <div className="thank-you-card">
                    <div className="thank-you-icon">
                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                            <polyline points="20 6 9 17 4 12"></polyline>
                        </svg>
                    </div>
                    <h1>Thank You</h1>
                    <div className="thank-you-message">
                        <p>Thank you for contacting TickSafe.</p>
                        <p>Our team has received your request and will contact you shortly.</p>
                    </div>

                    <div className="thank-you-actions">
                        <Link href="/news/tick-meat-allergy-australia" className="btn-home">
                            Back to Article
                        </Link>
                        
                        <div style={{ marginTop: '1rem', color: '#666' }}>
                            If your enquiry is urgent, please contact us directly:
                        </div>
                        <a href="tel:1300842572" className="btn-call-direct">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path>
                            </svg>
                            1300 842 572
                        </a>
                    </div>
                </div>
            </section>

            <footer className="thank-you-footer">
                <div className="container">
                    <p className="text-muted">
                        &copy; 2026 Tick Safe & The Mozzie Team. All rights reserved.
                    </p>
                </div>
            </footer>
        </main>
    );
}
